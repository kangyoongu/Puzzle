...
// Upgrade NOTE: excluded shader from DX11, OpenGL ES 2.0 because it uses unsized arrays
            ...
            float2 rayBoxDst(float3 boundsMin, float3 boundsMax, float3 boundsMin1, float3 boundsMax1, float3 rayOrigin, float3 rayDir){
                ...
                float3 tmin = min(t0, min(t1, min(t2, t3)));
                float3 tmax = max(t0, max(t1, max(t2, t3)));
            ...
            float4 frag (v2f input) : SV_Target
            {
                ...
                if(_Count <= 1)
                    ...
                    rayBoxInfo = rayBoxDst(_BoundsMin0, _BoundsMax0, _BoundsMin1, _BoundsMax1, rayOrigin, rayDir); 
